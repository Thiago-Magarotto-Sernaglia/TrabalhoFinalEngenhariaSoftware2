...
async def test_rota_obter_produto_por_id(mocker):
    ...
    mock_service = mocker.Mock()
    mock_service.obter_produto = AsyncMock(return_value={"id": 1, "nome": "Produto Teste", "preco": 10.0})
...
async def test_rota_obter_produto_nao_encontrado(mocker):
    ...
    mock_service = mocker.Mock()
    mock_service.obter_produto = AsyncMock(side_effect=HTTPException(status_code=404, detail="Produto não encontrado"))
